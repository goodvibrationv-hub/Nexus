...
ok('L2 — sous-titre nœuds maîtrisés', /10 nœuds maîtrisés sur 110/.test(R.lvlSub.textContent));
ok('L3 — barre de niveau à ~9-10%', /^(9|1[0-4])%$/.test(R.lvlBar.style.width||''));
...
ok('V1 — victoire nœuds maîtrisés 10/110', /10 \/ 110/.test(win)&&/nœuds maîtrisés/.test(win));
...
ok('G1 — nœuds à débloquer 100', /100<\/span><span class="l">nœuds à débloquer/.test(gap));
