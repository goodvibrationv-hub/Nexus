...
ok('L2 — sous-titre nœuds maîtrisés', /10 nœuds maîtrisés sur 103/.test(R.lvlSub.textContent));
ok('L3 — barre de niveau à ~10%', /^1[0-4]%$/.test(R.lvlBar.style.width||''));
...
ok('V1 — victoire nœuds maîtrisés 10/103', /10 \/ 103/.test(win)&&/nœuds maîtrisés/.test(win));
...
ok('G1 — nœuds à débloquer 93', /93<\/span><span class="l">nœuds à débloquer/.test(gap));
