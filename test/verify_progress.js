...
ok('L2 — sous-titre nœuds maîtrisés', /10 nœuds maîtrisés sur 72/.test(R.lvlSub.textContent));
ok('L3 — barre de niveau à ~14%', /width:\s*1[34]%/.test(R.lvlBar.style.width||'') || R.lvlBar.style.width==='14%' || R.lvlBar.style.width==='13%');
...
ok('V1 — victoire nœuds maîtrisés 10/72', /10 \/ 72/.test(win)&&/nœuds maîtrisés/.test(win));
...
ok('G1 — nœuds à débloquer 62', /62<\/span><span class="l">nœuds à débloquer/.test(gap));
